...
int main(int argc, char const* argv[]) {
	...
	try{
		...
		for(const std::pair<std::string, int>& oImagePathFlag : vImages) { // for each image
			const cv::Mat oInput = cv::imread(oImagePathFlag.first, oImagePathFlag.second); // load image
			if(oInput.empty()) {
