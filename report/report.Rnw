...
La décomposition en prises de vues est une méthode consistant à détecter les
...
de grande variation de lieu ou d'action. A terme l'objectif de cette
...
La décomposition en prises de vue est une méthode qui peut être abordée avec
l'utilisation d'histogrammes des vidéos que l'on décompose. Un histogramme,
...
représentera le nombre d'occurrence de chacune des valeurs de rouge de l'image
...
ayant une valeur rouge de 249 incrémentera le nombre d'occurrence du seuil 16.
...
La méthode des histogrammes vous a été brièvement présentée ici, même si ce
...
Une autre méthode --- celle que nous abordons dans ce TP --- pour effectuer la
décomposition en prises de vues consiste à comparer des arêtes. Cette fois ci
chaque trame n'est pas représentée par son histogramme mais par son gradient,
lorsqu'un gradient dépasse un certain seuil, on le considère comme un arrête.

Pour calculer ce gradient on applique une convolution entre des filtres et la
...
Pour déterminer de nouvelles arrêtes dans une prise de vue, on va chercher les
...
Si la proportion d'arrêtes entrantes et d'arrêtes sortantes est très élevée,
...
De cette façon on est donc suceptible de détecter tous les changements de
scènes. Mais avant de vous détailler notre méthode de détection, nous allons
...
Nous étudierons ici le détail des traitements réalisant la détection des
...
Les filtres de sobel sont deux matrices 3x3, l'une permettant la création d'un
...
Un fois que les grandiants en $x$ et en $y$ d'un pixel ont été calculés, la
norme (ou « force ») du gradiant final de ce pixel est simplement la somme de
leur valeur absolue.

Ainsi, en appliquant ces matrices sur chacun des pixels de l'image on obtient
l'image des normes des gradients associée, qu'on appelle aussi la carte des
forces des gradients.

Il faut juste remarquer que la valeur de la nomre d'un grandiant doit être
...
concervés tels quels dans la carte de la force des gradients normalisés.

De plus on précisera qu'ici notre implémentation effectue ce filtre sur chacun
...
Le seuillage de la carte consiste d'abord à extraire l'intensité de la
...
Ainsi l'image seuillée est une bitmap (une image binaire), c'est à dire une
image sur un canal dont les pixels sont soit tout noir, soit tout blanc. Les
arrêtes --- les normes des gradients combinés sur tois canaux dépassant le
seuil de luminosité --- sont donc les pixels blancs de l'image.

On remarquera que le calcule de l'intensité lumineuse consiste juste à faire
la moyenne des trois canaux d'un pixel. Dans notre implémentation, si cette
moyenne (donc l'intensité du pixel) est suppérieure à 127, alors on considère
...
des foces des gradients normalisés, on obtient notre image d'arrêtes.

\subsection{Dilatation des arrêtes}
\label{Dilatation arrêtes}

A présent que nous avons notre image d'arrêtes, il faut avoir les outils
...
les arrêtes de la trame précédente et de la trame courrante, afin de pouvoir
les comparer par la suite (les détails de cette comparaison seront abordés dans
la sous-section suivante).

Ainsi il faut être capable de dilater les arrêtes d'une image binaire
d'arrêtes dont la méthode d'obtenttion à été présentée précédemment.
...
d'arrivée. Le voisinage est d'une taille paramètrable, qui représente
finalement la taille de la dilatation des arrêtes.

Pour chaque pixel de l'image binaire d'entrée on parcourt donc tous leurs
...
Enfin on s'intéresse à déterminer quelle est le pourcentage d'arrêtes entrantes
ainsi que le pourcentage d'arrêtes sortantes entre deux images : l'image
courrante et l'image suivante. Pour déterminer si une arrête est entrante ou
sortante voici la méhtode logique à suivre :

\begin{itemize}
  \item Si une arrête de la trame suivante n'appartient pas ou « dépasse »
        des arrêtes dilatées de la trame courrante : alors c'est une arrête
        entrante.
  \item Si une arrête de la trame courrante n'appartient pas ou « dépasse »
        des arrêtes dilatées de la trame suivante : alors c'est une arrêtes
        sortante.
\end{itemize}

Calculer la proportion d'arrêtes entrantes et sortantes revient donc à :
...
Ainsi on peut déterminer la proportion d'arrêtes entrantes et sortantes entre
...
un peu plus élevée que la normale, alors une gradation ou un fondu est
probablement en cours.

De cette façon notre méthode de détection de changement de scènes via la
méthode des arrêtes est complète.

\subsection{Benchmarking}
\label{Benchmarking}

Toutes les étapes présentées précédemment on été implémentées de plusieurs
façons : sur un seul coeur / thread CPU, sur tous les coeurs / thread CPU
diponibles, sur la carte graphique, et via les méthodes openCV préexistantes.
Dans un soucis d'efficacité, la méthode finale ne retiendra que les
implémentations les plus performantes. Toutes les implémentations de toutes les
étapes seront comparées et selectionnées lorsque nous vous présenterons nos
résultas après avoir discuté des avantages et des inconvénients de notre
méthode.
...
La décomposition en prises de vues via les histogrammes (tel que décrit en
...
Ainsi la méthodes des arrêtes est plus efficace lorsqu'il s'agit de détecter un
un changement de scène lorsque ce changement est basé sur le mouvement (ou sur
...
Cela dit, une scène comportant beaucoup de mouvements et d'action risque d'être
...
De la même manière, une scène comportant beaucoup de variations de couleurs
...
Ainsi, chaque méthode présente ses avantages et ses inconvénients. Les
...
de mouvement / action semblent aussi plus fréquentes que les scènes comportant
