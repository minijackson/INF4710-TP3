...
TEST(Threshold, OpenCL) {
	...
	EXPECT_TRUE(assert_mat_equal(threshold_cl(test2, 127), expected1));
	EXPECT_TRUE(assert_mat_equal(threshold_cl(test2, 0), expected2));
	EXPECT_TRUE(assert_mat_equal(threshold_cl(test2, 2), expected3));
}
...
TEST(Threshold, DISABLED_Lena) {
	...
	cv::imshow("Lena (OpenCL)", threshold_cl(lena4, 127));
